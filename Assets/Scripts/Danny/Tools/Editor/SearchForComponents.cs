...
namespace SupportUtils
{
    ...
    {
        [MenuItem("Danny/Tools/Search For Components")]
        static void Init()
        {
        ...
        class ComponentNames
        {
            ...
            {
                return ((ComponentNames) obj).componentName == componentName &&
                       ((ComponentNames) obj).namespaceName == namespaceName;
            }
        ...
        void OnGUI()
        {
            ...
            switch (editorMode)
            {
                case 0:
                    ...
                    targetComponent =
                        (MonoScript) EditorGUILayout.ObjectField(targetComponent, typeof(MonoScript), false);
                    GUI.enabled = true;
                            ...
                            case 1:
                                ...
                                if (EditorApplication.SaveCurrentSceneIfUserWantsTo())
                                {
                                    ...
                                    foreach (string asset in allAssets)
                                    {
                                        ...
                                        {
                                            ComponentNames newComponent =
                                                new ComponentNames(NameFromPath(asset), "", asset);
                                            try
                                            {
                                                System.IO.FileStream FS = new System.IO.FileStream(projectPath + asset,
                                        ...
                                        if (indexPrefab != -1)
                                        {
                                            ...
                                            foreach (string dependedAsset in dependencies)
                                            {
                                                if (dependedAsset.IndexOf(".cs") != -1 ||
                                                    dependedAsset.IndexOf(".js") != -1)
                                                {
                                                    ComponentNames newComponent =
                                                        new ComponentNames(NameFromPath(dependedAsset),
                                    ...
                                    {
                                        addedComponents[i].assetPath = GetPathFromNames(
                                            addedComponents[i].namespaceName, addedComponents[i].componentName);
                                        ...
                                        foreach (GameObject g in sceneGOs)
                                        {
                                            ...
                                            foreach (Component c in comps)
                                            {
                                                    ...
                                                    c.GetType().BaseType == typeof(MonoBehaviour))
                                                {
                                                    ...
                                                    ComponentNames newComp = new ComponentNames(NameFromPath(path),
                                                        GetNamespaceFromPath(path), path);
            ...
            else
            {
                ...
                {
                    GUILayout.Label(
                        "The following components are not used by prefabs, by AddComponent, OR in any scene:");
        ...
        void DisplayResults(ref Vector2 scroller, ref List<ComponentNames> list)
        {
            ...
            foreach (ComponentNames c in list)
            {
                ...
                if (c.usageSource.Count > 1)
                {
                    GUILayout.Label("   In " + c.usageSource.Count + " Places: " + c.usageSource[0] + ", " +
                                    c.usageSource[1] + (c.usageSource.Count > 2 ? ", ..." : ""));
                }
            }
