...
namespace TheGame.GM
{
    public static class LuaToCsBridge
    {
        ...
        public static void LoadLuaConfigs()
        {
            ...
            CharacterTable = _luaEnv.Global.GetInPath<List<LCharacterConfig>>("Game.Designer.Character").ToDictionary(k => k.Id, v => v);
            ...
            ShopTable = _luaEnv.Global.GetInPath<List<LProductConfig>>("Game.Designer.Shop").ToDictionary(k => k.Id, v => v);
            Print($"[LuaConfigToCsInit] ShopTable {ShopTable.Count} success");

            ItemTable = _luaEnv.Global.GetInPath<List<LItemConfig>>("Game.Designer.Item").ToDictionary(k => k.Id, v => v);
            Print($"[LuaConfigToCsInit] ItemTable {ItemTable.Count} success");

            SummonTable = _luaEnv.Global.GetInPath<List<LSummonConfig>>("Game.Designer.Summon").ToDictionary(k => k.Id, v => v);
